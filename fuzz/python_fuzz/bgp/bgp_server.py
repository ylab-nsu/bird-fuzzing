        ...
        if pkt[IP].src == BIRD_BGP_ID:
            if str(pkt.summary()).find("BGPHeader") > 0:
                if pkt[BGPHeader].type == 1:
                    send(IP(dst=pkt[IP].src) / TCP(sport=pkt[TCP].dport, dport=pkt[TCP].sport, ack=pkt[TCP].seq + 1,
                                                   seq=pkt[TCP].ack, flags="PA") / BGPHeader(type=1) / BGPOpen(
                        version=4, AS=BIRD_ASN_ID, hold_time=PARAM_HOLD_TIME, bgp_id=HOST_BGP_ID))
                    return "Open Message sent"
                elif pkt[BGPHeader].type == 2:
                    send(IP(dst=pkt[IP].src) / TCP(sport=pkt[TCP].dport, dport=pkt[TCP].sport, ack=pkt[TCP].seq + 1,
                                                   seq=pkt[TCP].ack, flags="PA") / BGPHeader(type=4, len=19))
