...
class BgpUpdateFuzzer(BGFuzzTest):
    ...
    def create_bgp_open(self, n):
        ...
        with s_block("BGP"):
            ...
            with s_block("Open"):
                s_byte(
                    value=BGP_VERSION, endian=BIG_ENDIAN, name="Version", fuzzable=False
                )
                s_word(
                    value=self.FUZZER_ASN_ID,
                    endian=BIG_ENDIAN,
                    name="ASN",
                    fuzzable=False,
                )
                s_word(
                    value=self.PARAM_HOLD_TIME,
                    endian=BIG_ENDIAN,
                    name="Hold Time",
                    fuzzable=False,
                )
                s_dword(
                    value=self.ip_str_to_bytes(self.HOST_BGP_ID),
                    endian=BIG_ENDIAN,
                    name="BGP Identifier",
                    fuzzable=False,
                )
                s_byte(
                    value=0x00, endian=BIG_ENDIAN, name="Opt Parm Len", fuzzable=False
                )
    ...
    def update_test_fuzz_withdrawn_routes_length(self):
        ...
        with s_block("HEADER"):
            ...
            with s_block("UPDATE"):
                s_random(
                    ...
                    name="withdrawn_routes_length",
                    fuzzable=True,
                )
                with s_block("withdrawn_routes"):
                    s_static(name="prefix1_len", value=b"\x20")  # 32-bit length
                    s_static(
                        name="prefix1_addr", value=b"\xc0\xa8\x01\x01"
                    )  # 192.168.1.1
                s_size(
                    name="total_path_attr_len",
                    length=2,
                    block_name="FUZZLOAD",
                    endian=BIG_ENDIAN,
                    fuzzable=False,
        ...
        self.session.connect(
            s_get("BGP_KEEPALIVE1"), s_get("BGP_UPDATE_fuzz_withdrawn_len")
        )
    ...
    def update_test_fuzz_withdrawn_routes(self):
        ...
        with s_block("HEADER"):
            ...
            with s_block("UPDATE"):
                s_static(
                    name="withdrawn_routes_length", value=b"\x00\x08"
                )  # Static field (2 prefixes of 4 bytes each)
                with s_block("withdrawn_routes"):
                    s_byte(
                        name="prefix1_len", value=32, fuzzable=True
                    )  # Fuzzing prefix length
                    s_bytes(
                        name="prefix1_addr",
                        value=b"\xc0\xa8\x01\x01",
                        size=4,
                        fuzzable=True,
                    )  # Fuzzing prefix address
                s_size(
                    name="total_path_attr_len",
                    length=2,
                    block_name="FUZZ__LOAD",
                    endian=BIG_ENDIAN,
                    fuzzable=False,
                )
        ...
        self.session.connect(
            s_get("BGP_KEEPALIVE2"), s_get("BGP_UPDATE_fuzz_withdrawn_routes")
        )
    ...
    def update_test_fuzz_path_attributes_length(self):
        ...
        with s_block("HEADER"):
            ...
            with s_block("UPDATE"):
                s_static(
                    name="withdrawn_routes_length", value=b"\x00\x00"
                )  # Static field
                s_random(
                    value="",
                    min_length=2,
        ...
        self.session.connect(
            s_get("BGP_KEEPALIVE3"), s_get("BGP_UPDATE_fuzz_path_attr_len")
        )
    ...
    def update_test_fuzz_path_attributes(self):
        ...
        with s_block("HEADER"):
            ...
            with s_block("UPDATE"):
                ...
                s_size(
                    name="total_path_attr_len",
                    length=2,
                    block_name="path_attributes",
                    endian=BIG_ENDIAN,
                    fuzzable=False,
                )
                with s_block("path_attributes"):
                    s_byte(
                        name="attr_flags", value=0x40, fuzzable=True
                    )  # Fuzzing flags
                    s_byte(name="attr_type", value=0x01, fuzzable=True)  # Fuzzing type
                    s_byte(name="attr_length", value=1, fuzzable=True)  # Fuzzing length
                    s_byte(
                        name="attr_value", value=0x00, fuzzable=True
                    )  # Fuzzing value
                s_size(
                    name="total_path_attr_len2",
                    length=2,
                    block_name="FUZZ_LOAD2",
                    endian=BIG_ENDIAN,
                    fuzzable=False,
                )
        ...
        self.session.connect(
            s_get("BGP_KEEPALIVE4"), s_get("BGP_UPDATE_fuzz_path_attrs")
        )
        ...
        with s_block("HEADER"):
            ...
            with s_block("UPDATE"):
                ...
                with s_block("FUZZ-LOAD3"):
                    s_static(value=b"", name="Params")  # Empty field for attributes
                s_byte(
                    name="nlri_prefix_len", value=24, fuzzable=True
                )  # Fuzzing prefix length
                s_bytes(
                    name="nlri_prefix_addr",
                    value=b"\xc0\xa8\x02",
                    size=3,
                    fuzzable=True,
                )  # Fuzzing prefix address
